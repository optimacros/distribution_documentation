...
`mysql.config.template` - `string` позволяет настраивать конфигурационный файл my.cnf через передачу своего шаблона конфигурационного файла, который полностью заменит изначальный.
