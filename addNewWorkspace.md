...
Для этого переходим в директорию opt/ с помощью команды: `cd /opt`, затем создаём в ней директорию om/ с помощью 
команды: `mkdir om`. Переходим в om/ с помощью команды: `cd om` и затем создаём, находясь в ней, директорию workspace1/ 
с помощью команды: `mkdir workspace1` и рядом создаём директорию workspace-installer/ с помощью команды: 
...
Считаем ядра и видим, что их 6 на данном воркспейсе значит в `cpu` вводим значение 6. Теперь мы должны посмотреть
 значение свойства `memory`, которое соответствует количеству оперативной памяти сервера в байтах. Для того, чтобы это
...
Далее нам нужно узнать `workspace.id` его мы получаем, открыв логин центр и перейдя в раздел `Workspaces`. 
 
 ![](./pictures/workspaceId.png)
 
Чтобы узнать как нам заполнить поле `workspace.id`, нам нонадобится открыть логин центр и перейдя в раздел `Workspaces`.

![](./pictures/workspaceId.png)

Затем в адресной строке браузера копируем всё после последнего слэша. Чтобы впоследствии ввести это в поле 
`workspace.id` в файле манифеста. 
 
Все эти данные нужно ввести в соответствующие поля в файле манифеста, открыв его для редактирования вновь с помощью 
команды `nano manifest.json`. 

Так же не забываем ввести название воркспейса, который мы создаём, допустим `TEST` для 
тестового воркспейса или `PROD` для продакшн.
